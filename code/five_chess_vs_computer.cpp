...
int classical(){
	...
	if(chess[point+1][point]==HUMAN){//下
	...
	while(true){
		...
		if(m.uMsg == WM_LBUTTONDOWN){
			...
			if(row >= 0 && row < count && col >= 0 && col < count && chess[row][col] == 0){
				...
				if(flag){
					HWND wnd1 = GetHWnd();
					(MessageBox(wnd1, _T("恭喜你获胜了！"), _T("玩家获胜"), MB_OKCANCEL | MB_ICONQUESTION) == IDOK);
					return 1;
				}
				break;
			}
...
int score(int row, int col, int dir, int who){
	...
	string mystr = "zzzzzzzzz";  //左边4个右边4个
	switch(dir){
	case HENG:x=1;y=0;
		break;
	case SHU:x=0;y=1;
...
int getScore(int row, int col, int who){
	...
	int temp = 0;
	if(heng >= THREE && heng < FOUR)			//加重组合棋的分数
		temp++;
...
int AiGO(){
	...
	if(flag){
		HWND wnd1 = GetHWnd();
		(MessageBox(wnd1, _T("不好意思！你输了！哈哈哈哈哈哈哈哈哈哈哈！"), _T("玩家失败"), MB_OKCANCEL | MB_ICONQUESTION) == IDOK);
		return 1;
	}
	return 0;
}
...
restart:
	while(true){
		...
		createBoard();
		HWND wnd = GetHWnd();
		SetWindowText(wnd, "五子棋.Windlight_.");
		// flag2 = MessageBox(wnd, _T("是否使用递归预测模式？"), _T("提示"), MB_OKCANCEL | MB_ICONQUESTION) == IDOK;
		
		if(flag2)
			num_of_depth = depth2;
		else
			num_of_depth = 0;
		int flag = MessageBox(wnd, _T("是否让电脑先下？"), _T("提示"), MB_OKCANCEL | MB_ICONQUESTION) == IDOK;
		if (flag){
		...
		while(true){
			...
			ppp++;
			if(flag){
				int t = MessageBox(wnd, _T("加油哦，还差一点！重新试一次？"), _T("提示"), MB_OKCANCEL | MB_ICONQUESTION) == IDOK;	
				if(t)
					goto restart;
				exit(-1);
			}
